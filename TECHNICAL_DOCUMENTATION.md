...
8. On macOS, `PDFEditorNSDocument.save(to:ofType:for:completionHandler:)` asks `ContentView` to commit PDFKit's active AcroForm editor, lets the next-main-turn revision publication settle, and prepares a verified candidate. AppKit then writes that candidate using the requested Save or Save As operation. Only a successful write installs pending edits, clears the working unsaved flag, and updates the current document URL/title. On iOS, Save As writes the verified candidate to a unique temporary directory and presents `UIDocumentPickerViewController(forExporting:asCopy:)` through `NativePDFExportPresenter`; the picker delegate completes the same successful-save path with its returned destination URL, or clears the pending state and staging directory on cancellation.
