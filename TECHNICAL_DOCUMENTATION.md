...
### macOS Textbox sizing update (2026-09-07)

App-authored Textboxes use `PDFFormTextView`, a persistent multiline `NSTextView`, with a white `PDFFormTextBackgroundView` and no custom black border. The underlying Widget remains the serialized AcroForm field; selected fields retain the blue outline and corner handles. Page-overlay editors inherit page scaling; the fallback editor mounted directly in `PDFView` multiplies its font size by `scaleFactor` on creation and refresh.

`PDFFormDesignKind.fittedTextSize` is the shared macOS/iOS Textbox measurement. It uses the stored PDF font size, a 100-point minimum width, and explicit-line plus wrapped-line measurement. iOS matches `UITextView.textContainerInset` (six points on each horizontal edge and four on each vertical edge). macOS retains its three-point left/right frame inset and dynamically centers the AppKit layout vertically inside the existing overlay frame; this is presentation-only and does not alter serialized Widget bounds. Width and height are crop-clamped while retaining the left and top edges where possible; text edits and font-size changes remeasure both dimensions before committing.

New Textboxes begin at 100 × 22 PDF points, which accommodates the default 11-point text on both editors. While a Textbox is active, a font-size action immediately updates its editor font, adaptive bounds, outline and action bar without replacing uncommitted text, focus, or selection.

Verification: the user confirmed multiline input after the fallback font-scaling correction. The subsequent border removal and automatic-width changes have only static verification. Swift syntax parsing and `git diff --check` passed; an isolated AppKit measurement of two explicit lines returned width 103 points and height 26 points at 11pt. Xcode Build/Test was not run. Manual acceptance remains: long text growth, wrapping at the page edge, deleting back to one line, zoom changes, and Save/reopen.

`PDFViewPageChanged` and `PDFViewSelectionChanged` reconcile SwiftUI bindings on the next main-queue turn. Separate monotonic tokens and current PDFView, document, page and index identity checks discard stale callbacks, avoiding state changes during a SwiftUI view update, obsolete selection clears, and page flicker. A page change commits active inline text only after that validation.

## 1. System overview

PDF Editor uses an app-owned UIKit `UIDocumentBrowserViewController` on iOS and iPadOS and an app-owned AppKit `NSDocument` host on macOS. `PDFEditorDocument` remains the cross-platform mutation owner; `PDFEditorNSDocument` owns the macOS file lifecycle and explicitly opts out of autosave-in-place and permanent Versions history. The normal path uses:
...
| Path | Status | Notes |
| --- | --- | --- |
| `PDF_EditorApp` → platform document host → `PDFEditorDocument` → `ContentView` | Implemented | SwiftUI `WindowGroup` scene shell with an embedded, app-owned `UIDocumentBrowserViewController` on iOS/iPadOS; `PDFEditorNSDocument` on macOS. |
...
| Explicit save and iOS shared-document adoption | Implemented | macOS and iOS expose Save and Save As toolbar actions. On macOS, `PDFEditorNSDocument` presents the native panel, prepares and verifies the candidate, then performs the AppKit save operation. Identity changes only after success. It declares `autosavesInPlace`, `autosavesDrafts`, and `preservesVersions` false: unsaved changes still receive native close protection, but Browse All Versions and volume-version-storage warnings are disabled. iOS stages the verified Save As candidate in a unique temporary directory and presents `UIDocumentPickerViewController(forExporting:asCopy:)`; its delegate reports the selected destination, while either completion path removes the staging directory. Existing-file Save uses coordinated replacement, and staged-text installation shields PDFKit's page refresh while preserving the visible destination and scale. After the document browser reveals or imports an external PDF, iOS adopts a local copy into the visible Documents root and directs later explicit Saves there. |
...
```mermaid
flowchart TD
    App[PDF_EditorApp] --> Host[UIDocumentBrowserViewController on iOS or PDFEditorNSDocument on macOS]
...
1. **Application composition**: `PDF_EditorApp.swift` retains a SwiftUI `WindowGroup` scene shell on iOS/iPadOS so existing SwiftUI scene sessions remain restorable. `IOSDocumentBrowserRootView` embeds an app-owned `UIDocumentBrowserViewController`, forwards external URLs through `onOpenURL`, and the browser controller presents `ContentView` in a full-screen navigation controller without using the browser's document-transition controller. On macOS, `PDFEditorDocumentController` is installed before launch completion, suppresses AppKit document-window restoration, records explicit PDF-open requests, and instantiates `PDFEditorNSDocument`, which hosts `ContentView` and its `PDFEditorDocument` model.
2. **Presentation**: `ContentView.swift` and `FeatureViews.swift` own transient UI state, the Tools sidebar, toggleable Pages panel, viewing rail, Comment List, explicit Save and Save As actions, and document operations. The Tools sidebar starts visible on iPad and macOS, while iPhone opens a PDF with its Tools sheet hidden. On compact-width iOS, `ContentView` places the horizontally scrolling Save, Save As, Undo, Redo, Tools, and OCR controls in the native navigation toolbar and briefly overlays the filename at the top of the PDF; regular-width iPad and macOS layouts retain the native toolbar. On macOS, Open invokes `NSDocumentController`; File → Open Recent and the Tools sidebar share its recent-document list, while `PDFEditorNSDocument` owns the native title, represented URL, nonrestorable window controller, tabbing, Save/Save As operation, and close protection. Before display, the first document window sets `.preferred` tabbing, uses the public `NSWindowTabGroup.isTabBarVisible` state and `toggleTabBar(_:)` action to show the native tab bar even for one PDF, then synchronously hides the newly created New Tab control. A later PDF window is explicitly added to the main, visible, or first existing `PDFEditorDocumentWindow` through `addTabbedWindow(_:ordered:)` instead of creating another singleton group. It remains hidden until the normal `NSDocument.showWindows()` path displays and selects that tab once; `makeWindowControllers()` does not set `selectedWindow`, which would prematurely order the window and duplicate that presentation. An opened PDF window receives `NSScreen.main.visibleFrame`, completes its hidden SwiftUI/PDFKit layout and draws pending content before its window controller is added. While the host window is still invisible, `PDFKitHostView.layout()` performs the one-shot initial-page top navigation; this prevents a newly selected tab from first showing PDFKit's default position and visibly jumping after several main-actor turns. The existing key-window final-configuration notification remains a fallback when hidden layout does not yet have nonzero bounds or a loaded page. `PDFEditorDocumentWindow` synchronously hides the New Tab control immediately after AppKit adds a window to a tab group, before returning to the event loop. `WindowAttachmentView` repeats the same check synchronously on key-window and window-update notifications as a fallback for a rebuilt titlebar. The shared helper finds the titlebar view whose public accessibility role is `.tabGroup` and hides its direct `NSButton` child. Standalone AppKit probes confirmed that individually toggling two hidden windows creates two singleton groups, while explicitly adding the second window produces one two-window group; they also confirmed that assigning `selectedWindow` before `NSWindowController.showWindow(_:)` orders the second window twice, whereas leaving selection to the controller orders and selects it once. A one-window tab bar can hide its button before first display and remains visible when a two-window group returns to one window. The original File → New behavior remains enabled, and the implementation does not reference private AppKit class or selector names. `PDFEditorApplicationDelegate` schedules one native Open panel after the app first becomes active, covering ordinary launch and Xcode Run without relying on AppKit's default-launch flag or restoration timing; the deferred final check suppresses that panel when an explicit PDF open or existing document has arrived. A Dock/Finder reopen presents the panel only when no document or existing Open panel is visible, otherwise it restores existing document windows. A successful Save As lets AppKit update document identity; failure leaves the original identity and unsaved state intact. `PDFKitView.swift` bridges SwiftUI to `PDFView` on AppKit and UIKit.
...
- The UIKit document browser owns the iOS/iPadOS open, create, import, and active security-scope lifecycle; `PDFEditorDocument` remains the mutation and byte-snapshot owner. On macOS, `PDFEditorNSDocument` mirrors `EditorState` and pending inline edits into AppKit's change count, so native close protection reflects the actual working state while permanent Versions remain disabled.
...
| Path | Responsibility |
| --- | --- |
| `PDF Editor/PDF_EditorApp.swift` | Conditional application entry: a SwiftUI `WindowGroup` scene shell containing the app-owned document browser on iOS/iPadOS, and the SwiftUI/AppKit document host on macOS. |
| `PDF Editor/ContentView.swift` | Main editor UI, device-specific initial Tools presentation and toggleable Pages panel, compact-width iOS horizontal navigation toolbar and transient filename overlay, macOS recent-document state, native macOS Full Screen observation that hides and restores the toolbar, Tools sidebar, and narrow right-side viewing rail, Highlight and signature-placement state, Comment List integration, separate Save/Save As/Tools/OCR controls, imports/exports, OCR workflow, protected merge and Protect PDF flows, and transient view state. |
| `PDF Editor/Platform/IOSDocumentBrowserController.swift` | iOS/iPadOS SwiftUI scene bridge and `UIDocumentBrowserViewController` delegate; forwards external URLs, reveals/imports PDFs, creates blank documents, holds the active security scope, performs coordinated reads without an app-owned spinner or browser interaction lock, and presents/closes the SwiftUI editor without an animated document-browser transition. |
| `PDF Editor/Services/NativePDFExportPresenter.swift` | iOS-only `UIDocumentPickerViewController` bridge for manual PDF Save As export; reports the chosen destination or cancellation and prevents duplicate delegate completion. |
| `PDF Editor/FeatureViews.swift` | Pages panel and narrow viewing rail with Two Page, Two Page Scrolling, Fit Width, and Fit Page controls, plus direct page-number, total-page, separate previous/next, and Full Screen controls; Fit Width selects continuous scrolling before fitting each page to width, while Fit Page selects single-page presentation before fitting that page. On iOS/iPadOS Full Screen is a SwiftUI reading mode while macOS invokes the native window mode; English tool panels whose E-sign category exposes Fill in form fields, Add a signature, Add a checkmark, and Add a crossmark without Request e-signatures; the macOS Tools sidebar's bottom Recent files section with up to five cached Quick Look thumbnails; Protect PDF and Remove Password actions plus the protection password/confirmation form; reusable signature library, preview, deletion, and drawing pad; compact cross-platform PDF-image export options; multi-line Add Comment and Comment Editor views; Comment List; protected-merge password sheet; OCR result views; object inspector; and annotation inspector. |
...
| `PDF Editor/Services/PDFSigningIdentityService.swift` | Imports one password-protected PKCS#12 identity in memory, validates its certificate and signing key, and exposes only a short-lived signing identity plus review summary. |
| `PDF Editor/Services/PDFCMSService.swift` | Isolates the pinned Swift Certificates CMS SPI used to create and locally verify detached CMS signatures. |
| `PDF Editor/Services/PDFDigitalSignatureWriter.swift` | Strict classic-xref preflight and append-only signature transaction, including ByteRange, CMS Contents, field value, appearance, and final integrity checks. |
...
1. The platform document host supplies file bytes to `PDFEditorDocument`; the UIKit browser uses `init(data:)` on iOS/iPadOS, while the AppKit document host supplies the bytes on macOS.
   - On iOS/iPadOS, `LSSupportsOpeningDocumentsInPlace` remains enabled. The document browser reveals or imports the URL before it opens the editor. `ContentView` then runs the existing 300-millisecond, `fileURL`-identified adoption task for sources outside the visible Documents root so later explicit Saves target the adopted local copy.
...
8. On macOS, `PDFEditorNSDocument.save(to:ofType:for:completionHandler:)` asks `ContentView` to commit PDFKit's active AcroForm editor, lets the next-main-turn revision publication settle, and prepares a verified candidate. AppKit then writes that candidate using the requested Save or Save As operation. Only a successful write installs pending edits, clears the working unsaved flag, and updates the current document URL/title. On iOS, existing-file Save performs an `NSFileCoordinator` replacement write, while Save As writes the verified candidate to a unique temporary directory and presents `UIDocumentPickerViewController(forExporting:asCopy:)` through an always-mounted UIKit `NativePDFExportPresenter`; the picker delegate completes the same successful-save path with its returned destination URL, or clears the pending state and staging directory on cancellation. If successful staged-text installation replaces live PDFKit pages, `ContentView` supplies the selected page, the document posts a bounded display transition around page synchronization, and `PDFKitView.Coordinator` snapshots the old presentation, preserves the exact `PDFDestination`, scale and `autoScales` setting, redraws behind the shield for 0.20 seconds, then reveals the new presentation without a cross-fade.
...
2. `PDFPageObjectSnapshot` exposes type, bounds, composed transform, fill color, text/font properties, optional embedded font data, and image pixel size where applicable. Lightweight display enumeration omits font programs; macOS and iOS retrieve bytes only for the text object being activated and attach them to that editing snapshot.
...
1. Background page inspection retains lightweight geometry and display metadata without copying embedded font programs. When macOS or iOS activates an existing text object, `PDFEditorDocument.textObjectWithFontData` retrieves only that object's font bytes from the prepared editing session. The inline style retains the byte-backed `NSFont` or `UIFont` so zoom scaling does not require the font to be installed. Save-time replacement independently retrieves the selected object's original font data from the editing session.
...
On macOS, committing an existing-text inline edit keeps the first staged edit visible through an atomic live-editor-to-page-overlay handoff. If that page's overlay has not mounted yet, the live editor and its mask are retained only as a short-lived, noninteractive fallback; the fallback is removed when the mounted overlay takes over and is also removed on scrolling or document replacement. iOS provides the corresponding direct PDFView staged-text overlay, including re-edit hit testing and teardown cleanup. On both platforms, the original-text mask is aligned outward to device-pixel boundaries and expanded by one pixel on every side, and existing PDF text uses zero text-container inset rather than the FreeText editor's six-by-four-point padding. Platform layout aligns the first glyph's CoreText ink bounds and baseline with the original PDF text bounds; the live dashed selection outline follows the resized editor frame as characters are added or removed. iOS double-tap activation records the first touch in page space, makes PDFKit's competing double-tap recognizers wait, prefers a directly containing PDFium object, and bounds PDFKit word-selection fallback to the touched area. The iOS editor opens with a zero-length insertion cursor at the end of the text. When background object inspection is still pending, a nearby PDFKit word selection provides the editor frame and selection outline immediately; resolution later attaches the editor or its draft to the matching PDFium text object. If editing has already ended, resolution applies the draft without restoring a dismissed selection. This presentation-only handoff does not mutate the PDF or persist bytes; iOS does not auto-save inline edits.
...
- Existing AcroForm `/Widget` annotations use PDFKit's native controls and are omitted from general annotation move/delete snapshots. `PDFAcroFormService` records page/annotation references, names, types, read-only state, values, button states, choices, and export values. PDFKit hit/edit-completion notifications compare the state after native editing. `synchronizeAcroFormChangesIfNeeded` compares against the active PDFium export, or `sourceData` when no session exists; it serializes the presentation document, normalizes encryption, and verifies field states and page properties. `acroFormSessionIfRoundTripSafe` retains a PDFium session only if its export passes field and page checks. Otherwise the verified PDFKit bytes remain usable without that session. Synchronization retains the live `PDFDocument` identity and marks it unsaved, avoiding a full view reload after each value edit. General byte-snapshot Undo restores complete field state, distinct from bookmark-only Undo; authored-field deletion installs a complete verified canonical `PDFDocument` as described below. Save/Save As repeat synchronization for active editors, and both synchronous document writes and asynchronous manual-save preparation pass PDFium output through `PDFFormDesignService.registeredData` before persistence. Supported controls are text, checkbox, radio, combo, list, and signature fields; XFA and PDF JavaScript/calculation/submit actions remain unsupported. Applying a cryptographic signature uses the separate bounded transaction below and never passes its completed bytes back through the ordinary serialization path.
...
- Digital Signature Field uses the same direct placement, geometry, deletion, Undo/Redo, and canonical registration transaction with a 180 × 50-point default. It initially writes `/FT /Sig` and deliberately omits `/V` and `/DV`. Signature fields use generated `SignatureN` names, are excluded from ordinary value-edit controls, and do not trigger invalidation consent until a completed signature dictionary and ByteRange are present. Selecting an empty app-authored field exposes Digitally Sign in its canvas action bar. The signing sheet reads one password-protected `.p12` or `.pfx`, reviews the certificate holder, validity, algorithm and SHA-256 fingerprint, accepts an optional reason, then starts the signing transaction. Successful signing dismisses the sheet and immediately presents Save As so the source file is not overwritten implicitly.

- `PDFSigningIdentityService` limits PKCS#12 input to 16 MiB and requires exactly one identity. On macOS 15/iOS 18 and later it passes `kSecImportToMemoryOnly`; it never falls back to the legacy macOS behavior that imports into Keychain. It accepts RSA 2048-8192 or ECDSA P-256/P-384/P-521 signing keys, verifies the private/public key pairing, certificate validity and digital-signature key usage, limits the supplied chain, and retains no password or identity in document/preferences/Undo state. The project pins Apple Swift Certificates 1.20.0. All use of its `@_spi(CMS)` detached-signing API is isolated in `PDFCMSService`; this is package SPI rather than an Apple operating-system private API, but it remains a deliberately isolated source-compatibility risk.

- `PDFDigitalSignatureWriter` is append-only and limits input to 256 MiB. Its strict parser accepts classic cross-reference tables only, rejects encryption, hybrid/xref-stream files, XFA, duplicate or ambiguous structures, existing signatures, DocMDP/FieldMDP, field locks/actions and non-app-authored targets. It revises only the selected terminal Widget, AcroForm and Catalog, adds a Unicode-safe CoreText-outline appearance, `/Filter /Adobe.PPKLite`, `/SubFilter /adbe.pkcs7.detached`, UTC signing time, fixed-width `/ByteRange`, and a zero-padded 32 KiB `/Contents` reservation. Placeholder offsets are recorded while writing rather than rediscovered by searching PDF contents. Before returning, it requires the input prefix to be byte-identical, reconstructs the delivered signed ranges, verifies the embedded CMS against the selected leaf certificate as the explicit local anchor, and distinguishes that integrity result from public trust, revocation, timestamping or long-term validity.

- A completed app signature carries `/PDFEditorSigned true` in addition to its existing `/PDFEditorFormID`. `PDFFormDesignService` excludes that field from further authoring, while `PDFFormFieldTreeWriter` preserves its canonical field root and `/V` when other unsigned fields are rebuilt. `PDFEditorDocument` installs the result without creating a PDFium editing session and marks the exact signed bytes for verbatim Save/Save As. Any later content, annotation, form or security mutation clears that mode and remains subject to the signed-document invalidation warning.
- On iOS/iPadOS, an app-authored Textbox has a persistent, noninteractive `UITextView` display overlay attached to the `PDFView`. PDFKit's matching `/Widget` is hidden only for this app-authored Textbox identity; it remains the canonical serialized field. Selecting the Textbox promotes that same view to editable/selectable and commits its text and fitted PDF bounds through `updateAuthoredTextFormField` when editing ends. The overlay is re-laid out for PDF scale and visible-page changes, uses the stored PDF point size multiplied by the live PDFView scale, and is removed on document replacement or coordinator teardown. This avoids swapping between PDFKit's Widget renderer and UIKit while preserving Widget serialization, multiline flags, and the existing form-design verification/Undo path.
- The toolbar no longer exposes the original AcroForm draft designer; AcroForm creation is available through the Tools sidebar. Direct placement uses a 100 × 22-point Textbox default, longest-option-fitted Dropdown and List Box defaults with a 72-point List Box minimum height, and an 11 × 11-point Checkbox/Radio Button default. Dropdown reserves 32 points for native insets and its arrow; List Box reserves 36 points for native insets, its border, and PDFKit's scrollbar; both have a 48-point minimum and are crop-box clamped. Clicking an app-authored Widget selects it with the shared blue outline and four corner handles. A pan beginning on an unselected authored Widget selects it and starts the geometry interaction in the same gesture. A pan beginning inside the selected field moves its fixed-size bounds; a pan beginning within the adaptive corner-handle radius resizes from the opposite anchor. Button handles and their adaptive hit radius remain smaller so the 11-point fields retain a usable center drag region. Move and resize previews are crop-box clamped, and the final bounds use the same verified form-design transaction; a click or tap that does not become a pan still reaches PDFKit for button and choice-field filling (app-authored Textboxes use the platform-specific editor). On macOS, `PDFInteractionPDFView.hitTest` does not capture authored List Box interiors, allowing PDFKit's internal native selection view to receive option clicks; corner handles remain captured, and the PDFView parent pan recognizer retains authored-field movement. The macOS SDK maps its native choice-list control to a cell-based `NSTableView`. Before `super.hitTest` for an authored List Box, `PDFInteractionPDFView` temporarily applies Aqua appearance. After `super.mouseDown` creates the native table, it synchronously fixes Aqua appearance, white backgrounds and black text, switches the table from its automatic effective full-width style to `.plain`, and lays out the table and enclosing `NSScrollView` before the next display update. This removes the full-width cell inset without delaying the first correctly styled frame. The end-of-turn restoration and `.PDFViewAnnotationHit` callback retain the late-control fallback while the PDFView returns to its prior appearance. Native option selection and the app-owned editing selection remain active. App-authored Radio Button parents retain the Radio flag without `NoToggleToOff`, so clicking the selected option clears the group while selecting a sibling remains mutually exclusive. Selected Textbox, Dropdown and List Box fields show a compact action bar with the current point size, preset 8–48 point choices and Delete; Checkbox and Radio Button selections keep only their geometry handles. On macOS, the coordinator installs one window-scoped local key-down monitor while observing a PDFView and removes it during teardown or replacement. When the selected Textbox owns an editable native `NSTextView`, Delete and Forward Delete pass through for ordinary character editing; Escape moves first-responder status back to the PDFView while retaining the blue field selection. With no active Textbox editor, unmodified Delete (key code 51) or Forward Delete (117) is consumed before PDFKit's native Widget control, ends native form editing, hides the selection overlay, and invokes the same verified deletion callback as the action bar for any selected app-authored field. Command, Control, or Option combinations pass through. A Textbox text or font-size update preserves its left and top edges where page bounds permit and remeasures both width and height with the platform editor allowances before crop-box clamping. A Dropdown font-size update keeps its center and height, remeasures the longest option plus native control space, and crop-box clamps the fitted width. A List Box font-size update uses the same width fitting, fixes its top edge, and expands downward to the greater of 72 points or the measured height of all option rows plus vertical insets. All three preserve the live `PDFAnnotation` instance after the canonical candidate passes verification. Geometry Undo is named Move when only the origin changes and Resize when the size changes. Font presentation changes are prepared from the latest live Widget so unrelated PDFKit normalization is not mistaken for document drift, while the canonical session remains authoritative for persistence. Deletion first ends PDFKit's native Widget editing and constructs, serializes, reopens and verifies the complete candidate. It then assigns that canonical `PDFDocument` to the document model rather than removing a Widget from the displayed page. Textbox, Dropdown and List Box deletion Undo and Redo reopen and verify their canonical byte snapshots and install those complete documents in the same way, preserving the catalog, `/AcroForm/Fields`, pages and Widgets as one consistent unit. On macOS, `PDFKitHostView` keeps the fully rendered outgoing `PDFView` above a second live replacement view. The incoming view receives the canonical document, viewer mode, current destination, scale and auto-scale state, then remains attached beneath the outgoing view for 0.40 seconds so PDFKit can build its page and Widget tiles. AppKit cross-fades the outgoing view to the prepared replacement over 0.12 seconds and removes it without invalidating the host. A generation counter cancels obsolete staging work if another document arrives or the representable is dismantled. The coordinator transfers observation, gestures and overlays to the incoming view only after promotion. This avoids incrementally deleting a native Widget, preserves a consistent field tree, and prevents the stale black frame plus blank/partial replacement frame seen with direct visible-document assignment and bitmap shielding. Crop origins and page rotation are handled by `PDFFormPageGeometry`; changing a radio selection clears siblings. Sidebar expansion preferences retain the v1-to-v2 migration. The sidebar entry remains unavailable for locked or empty documents and while saving or running OCR.
...
- On iOS/iPadOS, FreeText selection resolves every single-finger drag as `.move` and rejects pinch scaling because its measured contents and text settings determine its bounds. Ink selection instead uses the shared corner hit test: dragging inside the blue outline moves the annotation, dragging one of its four corner handles scales it through the existing bounds transform, and the separate pinch scale gesture remains available. A repeat single tap on the selected FreeText annotation dispatches `beginInlineTextEditing` rather than relying on PDFKit's text-selection double tap. Existing FreeText initializes `UITextView.selectedRange` to the UTF-16 end offset with length zero; new drafts retain all-text selection. `keyboardWillChangeFrameNotification` stores the screen keyboard frame. The coordinator converts it through the window into PDFKit's recursively located document `UIScrollView`, converts the editor rect from the editor view itself, and applies the minimum bounded vertical offset needed to keep its lower edge 12 points above the keyboard. A next-main-actor layout refresh updates the direct-child editor after PDFKit scrolls its document subview.
- Add a signature presents the app-scoped signature library rather than immediately mutating the current page. In compact iOS layouts, ContentView dismisses the Tools sheet first and presents the library only from that sheet's dismissal callback. Templates store validated 0...1 normalized vector points in an atomically written JSON file under Application Support, with limits of 64 templates, 128 strokes and 16,384 points per template, and a 512 KB total payload. iOS applies complete-until-first-authentication file protection. Corrupt, oversized, or invalid data loads as an empty library with a visible error instead of entering the PDF document lifecycle. Choosing a template enables a one-shot canvas placement mode. On macOS, `mouseMoved` converts the pointer to page space and renders the selected normalized strokes in a translucent high-z-position `CAShapeLayer`; the preview is hidden outside a page, during scrolling, after cancellation, and immediately before commit. Preview and commit both call `SignaturePlacementGeometry`, so the displayed 200-by-80-point crop-box clamping matches the final ink position. At commit, `PDFAnnotationService` maps all strokes into page space, computes a shared tight ink rectangle with two-point padding, and rewrites every path into coordinates local to that rectangle. The selected blue dashed outline therefore hugs the visible signature while PDFKit can still preserve its geometry, color, and line width across serialization and reopen. New signatures use a fixed opaque black rather than macOS's appearance-dependent `labelColor`; Ink palette choices also normalize opacity to one because PDFKit regenerates opaque Ink appearance streams. Highlight palette choices continue to preserve their annotation opacity. iOS retains direct tap placement, single-finger movement, corner-handle resizing, and pinch scaling after placement. The shared PDFView coordinator converts the commit click or tap into a page index and PDF-space point; ContentView completes the existing annotation mutation/signature-consent transaction and selects the new annotation for the existing move/resize/delete controls.
...
- Highlight remains enabled without an initial selection. When Tools opens, `ContentView` snapshots a valid current `PDFSelection`; otherwise choosing Highlight enables a selection banner. Compact iOS layouts dismiss the modal Tools sheet before entering Highlight mode so the banner and PDF text selection surface are visible and interactive; wide layouts retain the inline Tools panel. Apply Highlight uses Return, Cancel uses Escape, and applying requires non-whitespace selected text with nonempty bounds. `PDFAnnotationService` converts each PDFKit quadrilateral from page coordinates to annotation-local coordinates by subtracting the annotation bounds origin before assigning `quadrilateralPoints`.
...
- Existing PDFs are written only by explicit Save. On iOS, Save As presents the UIKit export destination picker with a temporary verified PDF; new-document Save and later Save continue to use the selected URL.
- Committed document mutations use complete serialized PDF `Data` snapshots for Undo/Redo. On iOS, `EditorState` owns the `UndoManager` used by `ContentView`, so the history follows the document lifetime and is not released as part of the browser transition. macOS continues to use the environment Undo manager supplied by AppKit. Pending inline replacements use value-level store actions until Save applies them. Textbox deletion installs the complete verified snapshot document for Delete, Undo and Redo rather than reconstructing the AcroForm field tree through page insertion.
...
Explicit replacement saves propagate file-coordination or atomic-write errors to the same operation alert and retain the unsaved working state. Cancelling the iOS native Save As destination picker is treated as cancellation rather than an error, removes the temporary export directory, and leaves the document unsaved.
...
- The UIKit document browser owns the iOS/iPadOS open/new lifecycle and holds the active document's security scope until the editor closes. iOS manual Save As uses a UIKit destination-picker bridge, while existing-file persistence is an explicit security-scoped, file-coordinated replacement initiated from Save.
...
- `FormDesignRoundTrip.swift`: opt-in production-service validation for text (including Chinese/multiline), checkbox, radio, Dropdown, List Box, and Digital Signature Field creation/reopen, canonical field registration, default/geometry changes, deletion including the final authored field, preservation of an unmarked existing field, invalid names/export/group rejection, rotated/nonzero-crop geometry, default sizes, choice-field fitting, identity-preserving live Widget updates, distinct verified canonical documents for deletion/restoration/redo, and choice options/current/default persistence. Its XMP fixture also verifies that canonical registration restores `/Filter /FlateDecode` when PDFKit serializes a compressed Metadata stream without that filter.
- `FormDocumentPlacementRoundTrip.swift`: opt-in full-document regression validation that creates and edits Dropdown, List Box, Textbox, and Digital Signature fields through `PDFEditorDocument`, covering Undo, deletion restoration, geometry and font fitting. Its final-save case starts with XMP Metadata, adds an empty Signature Field, follows the asynchronous `prepareManualSave` path used by Save/Save As, and requires readable XMP plus the same canonical empty `/FT /Sig` field after reopen.
- `PDFDigitalSignatureRoundTrip.swift`: opt-in signing validation that constructs a minimal app-authored empty Signature Field, rejects unsupported or ambiguous inputs, signs it with a supplied disposable PKCS#12 identity, checks prefix preservation and ByteRange/CMS integrity through an independent Core Graphics path, rejects a second signing attempt, and confirms covered-byte tampering fails verification. It writes unsigned, signed and unchanged signed-copy fixtures for external pyHanko and rendering checks.
...
Manual app acceptance: create a blank PDF; choose Tools → Acroform → Textbox and verify that no design window opens. Click for the default size and drag for a custom rectangle. Escape before/during a drag must create no field. Verify one placement per gesture, immediate return to native filling, Undo/Redo, Save As/reopen and Chinese input. Repeat for Checkbox and individual Radio Button options, using the same Group Name for exclusion and a new name for another group. Repeat for Dropdown with short, long and Chinese options and verify that a click-created field follows the longest line plus native control space, stays inside the page, and retains an explicitly dragged width. On iOS/iPadOS, verify that the app-owned Dropdown overlay immediately displays a first direct selection of Option 1, opens its action sheet, moves without scrolling the PDF page, responds to the point-size control, and changes its Options list one line at a time; verify long options refit the field, invalid current/default values clear, and the selection/options survive Undo/Redo and Save As/reopen. For iOS List Box, verify that every option is simultaneously visible as a separate row, tapping a row immediately selects and highlights it, and dragging moves only the field; also verify font-size and Options changes, Save As/reopen, and Undo/Redo. Repeat for List Box with short, long and Chinese options; verify longest-option width fitting with a 72-point minimum height and downward growth for larger fonts, direct native selection of individual option text, empty/current/default states, movement, corner resizing, page action-bar font changes, Delete/Undo, and persistence. Repeat on scanned PDFs so the image behind a field does not intercept filling, plus rotated/nonzero-crop pages, encrypted/restricted documents and signature consent. Check toolbar/sidebar cursor transitions, compact Tools dismissal, scrolling/zoom, and iOS touch navigation. On a compact-width iPhone, verify that the PDF opens without presenting the Tools sheet, the filename appears once over the PDF and disappears after three seconds, and the native navigation toolbar exposes Save, Save As, Undo, Redo, Tools, and OCR, scrolls horizontally when required, preserves each disabled state, and transitions correctly across rotation; confirm that a regular-width iPad continues to use the native toolbar and initially visible Tools sidebar.

Digital Signature Field acceptance: choose Tools → Acroform → Digital Signature Field, place or drag the 180 × 50-point empty field, move/resize it, exercise Delete/Undo/Redo, then Save As and reopen. Confirm that the canonical field is `/FT /Sig`, has no `/V`, remains connected to its page Widget, and does not trigger signed-document invalidation consent. Select the field, choose Digitally Sign, import a one-identity `.p12`/`.pfx`, review its certificate, sign, and complete the automatic Save As. Reopen the result and independently verify its CMS digest, exact ByteRange, whole-file coverage, field/Widget relationship and appearance. An unchanged second Save As must retain validity; changing one covered byte must fail verification. Wrong password, cancellation, absent/mismatched key, expired/non-signing certificate, existing signature, encrypted or unsupported xref input, MDP/lock/action, oversized input/chain/CMS, concurrent document change and output cancellation must not alter the source PDF or install a purported signature.
...
### iPhone viewing-control overlay

`ContentView.usesPhoneViewerControls` selects the phone device idiom, independently of horizontal size class. Both layout branches omit the fixed viewing rail on iPhone and attach `PDFPhoneViewerControls` as a trailing overlay. The 52-point controls and 24-point handle do not participate in PDF width allocation, so showing or hiding them does not trigger PDF layout resizing. iPad and macOS retain the fixed rail.

`PDFPhoneViewerControls` starts expanded. Its full-height handle area accepts a tap or an edge drag; a horizontal-dominant drag exceeding 24 points reveals the controls when directed left and dismisses them when directed right. The simultaneous drag gesture is confined to the handle and controls. `PDFRightPanel` reports button actions, page-number draft changes, and focus changes through callbacks. An interaction revision restarts a cancellable three-second SwiftUI task; dragging, page-number focus, or VoiceOver prevents automatic hiding. Page-number focus also blocks manual dismissal. Reduce Motion suppresses the normal 0.2-second transition. SwiftUI cancels the task when the overlay disappears. No PDFKit gesture-bridge changes were required.

### Verified in this task

On 2026-09-12, the user ran the complete built-in Digital Signature workflow on macOS: created and selected `Signature1`, reviewed a one-day disposable RSA 3072-bit self-signed `.p12`, signed in the app and completed the automatic Save As to `OBM Timetable 2026.09.11 3.pdf`. Independent pyHanko 0.37.0/pyhanko-cli 0.5.0 validation reported a cryptographically sound SHA-256/RSA signature, whole-file coverage and `VALID` when the same self-signed certificate was supplied as the explicit trust root. Structural inspection confirmed the canonical filled `/FT /Sig` field, `/Adobe.PPKLite`, `/adbe.pkcs7.detached`, exact whole-file ByteRange, 32 KiB Contents reservation, connected page Widget and nonempty appearance. Poppler rendered both pages at 144 DPI; the visible signer name and UTC time were clear and unclipped, and no page, table or glyph damage was observed. The final PDF SHA-256 was `f10e84ad483287ff5eaf10d33f2e542ea5c28fc1358baddd48f821fabe41abf8`. This validates the tested built-in signing path and PDF integrity, but the disposable certificate does not establish public trust, revocation status, a trusted timestamp or long-term validity. The isolated pyHanko environment, `.p12`, certificate and private key were deleted after verification. Repository-wide application Swift syntax parsing, per-file Validation script syntax parsing, focused CoreGraphics/Security API checks, project-file plist lint and `git diff --check` passed. Xcode compilation errors encountered by the user were corrected by importing the defining `SwiftASN1` module, using the current fixed-expiry SPI API, and calling the static writer method; Codex did not run an Xcode Build/Test, Simulator or device validation.

On 2026-09-11, a user-run macOS Save As of `OBM Timetable 2026.09.11 2.pdf` confirmed that the final asynchronous manual-save path retained an empty canonical `Signature1` `/FT /Sig` Widget and emitted its compressed XMP Metadata with `/Filter /FlateDecode`. Codex then used pyHanko 0.37.0 with pyhanko-cli 0.5.0 and `--existing-only --field Signature1` to sign that exact saved input directly, without a preprocessing repair. pyHanko reported `Signature1:FILLED`, a cryptographically sound SHA-256/RSA detached signature, whole-file coverage, and an overall `VALID` judgment when the one-day self-signed test certificate was supplied as the trust root. Reopen checks retained the field/Widget relationship, nonempty appearance, readable XMP, and both rendered pages. The original input hash remained unchanged; the temporary private key and isolated pyHanko environment were deleted. Swift syntax parsing and `git diff --check` passed for the implementation, but Codex did not run an Xcode Build/Test or Simulator validation.

On 2026-09-09, the macOS existing-PDF-text editor separated FreeText padding from ordinary text, made its dashed selection frame track the live editor size, hydrated only the activated object's embedded font, retained that byte-backed font across zoom scaling, extended the original-text mask by one backing pixel, and aligned the first glyph's CoreText ink edge with the original bounds while preserving baseline alignment. User-provided screenshots established the preceding horizontal offset, fixed-size outline, font substitution, and one-pixel left-edge remnant; the final combined behavior still requires manual macOS verification. Swift syntax parsing, focused AppKit/CoreText API checks, and `git diff --check` passed. No Xcode Build/Test, Simulator, or automated UI validation was run.

On 2026-09-09, the iOS/iPadOS document entrance changed from `DocumentGroup` to an app-owned `UIDocumentBrowserViewController` embedded in a SwiftUI `WindowGroup`. The retained SwiftUI scene shell supersedes an intermediate UIKit-only app/scene delegate implementation whose restored legacy SwiftUI session showed a black screen and crashed in `SwiftUI.AppSceneDelegate.sceneItem()` with `Fatal error: Missing scene item!`. A user-provided Simulator screenshot then confirmed that the new system document-browser page displayed. After the user observed the app-owned rotating activity indicator during document selection, that indicator and the browser interaction lock were removed while coordinated reading, duplicate-open protection, security-scoped access, and nonanimated editor presentation remained. iOS/macOS Swift syntax parsing, focused iOS type checking, plist lint, and `git diff --check` passed. Xcode Build/Test was not run by Codex. Document selection without the spinner, PDF open/edit/save, external-file adoption, Back without a thumbnail transition, iPad layout, and physical-device behavior remain manual runtime checks.

On 2026-09-05, the iPhone viewing-control overlay passed iOS- and macOS-targeted Swift syntax parsing and `git diff --check`. Parsing is not full type checking; the iOS parse emitted a host macOS sysroot warning. No Xcode Build/Test, Simulator, or device UI validation was performed. Manually verify right-edge reveal, rightward dismissal, handle taps, idle timing after control use and dragging, page-number focus protection, PDF scrolling/pinch zoom, portrait/landscape safe areas, VoiceOver, Reduce Motion, and the fixed iPad/Mac rail.

On 2026-09-05, the compact-width iOS horizontal navigation toolbar, one-time filename overlay, and iPhone-hidden initial Tools sheet passed Swift syntax parsing plus `git diff --check`. No Xcode Build/Test, Simulator run, or device UI validation was performed. Exact navigation-bar placement, filename timing, horizontal scrolling feel, disabled-state refresh, rotation, Dynamic Type, initial iPhone/iPad presentation, and regular-width iPad fallback remain manual UI checks.

On 2026-09-05, the iOS document-owned Undo manager and `DocumentTitleMenuDisabler` passed iOS/macOS Swift syntax parsing, a focused iOS helper type check, and `git diff --check`. Earlier device checks showed that clearing only the rename delegate disabled the control without removing its appearance. The bridge therefore waits one main-actor turn after editor appearance, then clears the rename delegate, title-menu provider, and document properties after `DocumentGroup` has configured them. No Xcode Build/Test or simulator validation was run. The final absence of the title-menu/Rename button and the responsiveness of the transition back to the document browser remain physical-device checks.
...
- On 2026-09-06, an iPhone Instruments trace showed that the prior SwiftUI Save As exporter path completed its outer preparation in 27.81 ms, including 24.92 ms for `PrepareManualSave` and 9.11 ms in PDFium-locked preparation; it was therefore not a PDFium stall despite the endlessly visible toolbar spinner. The SwiftUI sheet-hosted picker could then show a blank window and immediately dismiss. iOS Save As now stages the verified candidate in a unique temporary directory and uses an always-mounted UIKit host to explicitly present `UIDocumentPickerViewController(forExporting:asCopy:)` through `NativePDFExportPresenter`, retaining duplicate completion protection and removing that directory on success or cancellation. `git diff --check` and Swift syntax parsing passed. The native picker, selected-destination persistence, cancellation, and follow-up Save require manual iPhone/iPad verification; no Xcode Build/Test was run.
...
- On 2026-09-01, user-run macOS testing confirmed that app-authored Textbox Delete, Undo, and Redo complete through the two-live-`PDFView` replacement without the prior stale black frame or visible blank-page flash. The final source removed the temporary bitmap marker, pixel sampling, opacity experiment, and `[AcroFormDisplay]` timing output. Debug and Release standalone type checks plus `git diff --check` passed afterward. This evidence is scoped to the tested PDF, zoom and page mode; broader PDF compatibility, Save/Save As/reopen, and iOS behavior remain separate verification work. Codex did not run an Xcode Build/Test for this update.
- On 2026-09-06, iOS Checkbox selection gained a field-sized transparent UIKit capture view because PDFKit's native Widget consumed PDFView-level drag gestures. Its dedicated `UIPanGestureRecognizer` records the first `touchesBegan` location, rather than using the already-displaced `.began` pan location, then reuses the existing form move/resize transaction and bounds callback. The capture view exists only while an app-authored Checkbox is selected and is removed on deselection, deletion, document teardown, or non-Checkbox selection. Checkbox Delete is available from the selected-field action bar without a font-size control. Swift syntax parsing and `git diff --check` passed; user-run iOS testing confirmed Checkbox movement. No Xcode Build/Test was run by Codex.
- On 2026-09-06, that iOS capture-view path was generalized to both button Widget kinds (`.isButton`): Checkbox and Radio Button. The same original-touch capture now reaches the existing bounds transaction after a selected Radio Button is dragged, while first taps remain with PDFKit so a Radio Button's native group selection and toggle-to-off behavior are unchanged. Swift syntax parsing and `git diff --check` passed; Radio Button device interaction remains a manual verification item. No Xcode Build/Test was run by Codex.
- On 2026-09-06, iOS app-authored Dropdowns moved from PDFKit's native choice control to an app-owned UIKit label-and-chevron overlay with an action-sheet choice picker, because direct Simulator instrumentation showed the first native option emitted a `UITextField` event while the Widget value remained empty. The overlay writes the Widget value directly, participates in the existing form move transaction, and makes PDF scroll pans wait for a field drag. The selected-field action bar now edits Dropdown/List Box options one per line; changing options refits the Dropdown and removes invalid current/default values. AcroForm design application now retries with `PDFFormFieldTreeWriter` whenever a PDFKit candidate fails field retention, accepting the fallback only after field and reopen verification. `git diff --check` and Swift syntax parsing passed. User-run Simulator testing previously confirmed immediate first Option 1 display and long-option width fitting; final overlay dragging, font-size resizing, choice editing, and the new canonical fallback remain manual verification. No Xcode Build/Test was run by Codex.
- On 2026-09-07, iOS app-authored List Box fields received a separate UIKit stack overlay rather than the Dropdown's single-line action-sheet control. It renders every option as a visible row, writes the tapped row directly to the serialized Widget, highlights the selected row, and shares the existing field-drag versus PDF-scroll coordination. Dropdown remains a single-line label-and-chevron action-sheet control. `git diff --check` and Swift syntax parsing passed; row layout, hit selection, dragging, font-size/Options changes, Undo/Redo, and Save As/reopen remain manual Simulator verification. No Xcode Build/Test was run by Codex.
- On 2026-09-09, iOS/iPadOS Ink annotations, including newly placed signatures, began routing blue-corner-handle drags through the existing annotation scale transform while retaining body-drag movement and pinch scaling. Swift syntax parsing and `git diff --check` passed. Corner targeting, movement-versus-resize behavior, PDF zoom coexistence, Undo/Redo, and Save/reopen remain manual device checks; no Xcode Build/Test or simulator validation was run by Codex.
- On 2026-09-10, iOS existing-PDF-text editing gained on-demand embedded-font hydration, zoom-aware byte-backed UIKit fonts, zero content inset, CoreText ink-edge/baseline alignment, live editor and dashed-outline resizing, one-device-pixel-expanded masks, and re-editable staged overlays. Direct and fallback text hits now use deterministic proximity rules, the app-owned double-tap activation prevents simultaneous PDFKit zoom and retains its initial page-space touch, and the initial insertion cursor is placed at the end. User-run Simulator checks confirmed that double-tapping the `Created` run selected the intended object after the gesture fix; cursor-at-end behavior, broader PDFs, zoom/scroll interaction, Undo/Redo, and Save/reopen remain manual checks. Swift macOS/iOS syntax parsing, focused UIKit/PDFKit API type checks, and `git diff --check` passed; Codex did not run an Xcode Build/Test.
...
- `git diff --check`: passed.
- Exact toolbar appearance, thumbnail dragging, page-action buttons, responsive panel sizing, and the app-owned iOS document-browser open/close presentation still require manual UI validation.
