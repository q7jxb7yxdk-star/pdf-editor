...
- PDFKit creates notes, free text, highlights, signatures, and freehand Ink annotations. Draw freehand enables a dedicated pan gesture on the PDF canvas, renders an immediate `CAShapeLayer` preview in view coordinates, converts the completed points into page coordinates, creates one Ink annotation, exits drawing mode, and selects the new annotation.
