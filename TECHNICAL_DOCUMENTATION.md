...
2. **Presentation**: `ContentView.swift` and `FeatureViews.swift` own transient UI state, the hidden-by-default Tools panel, toggleable Pages panel, viewing rail, Comment List, explicit Save and Save As actions, and document operations. On macOS, Open invokes `NSDocumentController`; `PDFEditorNSDocument` owns the native title, represented URL, window controller, tabbing, Save/Save As operation, and close protection. A successful Save As lets AppKit update document identity; failure leaves the original identity and unsaved state intact. The startup window attachment still performs the one-time filled-window layout after the native Open panel closes. `PDFKitView.swift` bridges SwiftUI to `PDFView` on AppKit and UIKit.
...
- Existing AcroForm `/Widget` annotations use PDFKit's native text, button, and choice controls and are deliberately omitted from `PDFAnnotationService` snapshots so the app's general annotation hit testing cannot move, resize, restyle, or delete a form field. `PDFAcroFormService` records stable page/annotation references plus field name, field/control type, read-only state, value, button state name, choices, and export values. `PDFViewAnnotationWillHit` captures a baseline; `PDFViewAnnotationHit` and platform text-edit completion notifications compare the current form state after PDFKit has applied the native edit. A real change asks `PDFEditorDocument` to serialize the presentation document, normalize its encryption policy, open that candidate as the new PDFium editing session, export it, and reopen it with PDFKit to verify all non-pushbutton field states. The mutation uses the shared byte-snapshot Undo/Redo and digital-signature-consent paths and marks the document unsaved. Save and Save As repeat the comparison against the current PDFium bytes before preparation, covering a text field that is still first responder when Save is invoked. This supports existing AcroForm text fields, checkboxes, radio buttons, combo boxes, and list boxes; it does not execute XFA, PDF JavaScript/calculation/submit actions, author new fields, or create cryptographic signatures for Signature Widgets.
...
- `AcroFormRoundTrip.swift`: creates text, checkbox, radio-button, and choice Widgets and verifies their values after PDFKit serialization and reopen.
