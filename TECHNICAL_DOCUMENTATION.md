...
| Explicit save | Implemented | macOS and iOS expose Save and Save As toolbar actions. macOS presents `NSSavePanel` immediately, begins isolated candidate preparation while the filename panel remains open, and also replaces File → Save and File → Save As…, with Command-S and Shift-Command-S shortcuts. When the panel confirms a destination, the native macOS document synchronously adopts it before waiting for preparation or file writing; the title, Window menu, represented URL, and later Save target therefore change without PDF-regeneration latency. Cancellation keeps the previous location, while preparation or write failure rolls the immediate adoption back. iOS uses the SwiftUI file exporter with independent presentation state. Inline text edits remain in a `PendingTextEditStore` until a successful save. |
...
2. **Presentation**: `ContentView.swift` and `FeatureViews.swift` own transient UI state, the hidden-by-default Tools panel, toggleable Pages panel, narrow right-side viewing rail, Comment List, explicit Save and Save As actions, and document operations. In wide layouts, the Tools toggle updates panel visibility without `withAnimation`, preventing the central `PDFView` from being resized across multiple animation frames while `autoScales` is active. The viewing rail places direct page-number input, a total-page display, and separate previous/next navigation controls after Zoom out. Save, Save As, Undo, Redo, Tools, and OCR use separate leading toolbar items with hidden shared backgrounds and invoke focused document actions where applicable. `DocumentGroup` owns the initial native document title. When a macOS Save As panel confirms, `ContentView` captures the previous native identity, immediately rebinds the hosting `NSDocument` and `saveURL`, and explicitly synchronizes the `NSWindow` title, represented URL, and Window menu item before awaiting PDF regeneration or writing. Successful saving retains that destination; failure restores the captured identity. The startup window attachment records actual `NSWindow` identities in a weak table and applies a one-time, non-animated fill frame as soon as the window attaches, preventing a `DocumentGroup` URL refresh from resizing the same window again during layout. `PDFKitView.swift` bridges SwiftUI to `PDFView` on AppKit and UIKit.
...
| `PDF Editor/ContentView.swift` | Main editor UI, hidden/toggleable Tools and Pages panels, narrow right-side viewing rail composition, Highlight and signature-placement state, Comment List integration, separate Save/Save As/Tools/OCR toolbar items, imports/exports, OCR workflow, protected merge flow, and transient view state. |
| `PDF Editor/FeatureViews.swift` | Pages panel and narrow viewing rail with display, zoom, direct page-number, total-page, and separate previous/next controls; English tool panels whose E-sign category exposes Add a signature, Add a checkmark, and Add a crossmark without Fill & Sign or Request e-signatures; reusable signature library, preview, deletion, and drawing pad; compact cross-platform PDF-image export options; multi-line Add Comment and Comment Editor views; Comment List; protected-PDF password sheet; OCR result views; object inspector; and annotation inspector. |
...
| `PDF Editor/Services/ManualPDFSaveCoordinator.swift` | Security-scoped, `NSFileCoordinator`-protected replacement writes plus focused macOS File → Save and File → Save As… commands. |
| `PDF Editor/Services/PDFManualSavePreparationService.swift` | Builds pending-text Save candidates on an independent background session, selects the content rewrite or PDFKit fallback per page, and verifies the reopened result before publication. |
...
| `PDF Editor/Services/PDFPageResourceIntegrityService.swift` | Reads page ColorSpace, Pattern, and Shading resource counts, fail-closes special-resource text replacement, and rejects candidates that lose those resources. |
...
7. Leaving an inline text editor records or removes a `PendingTextEdit` in `PendingTextEditStore`; it does not invoke PDFium. The store registers symmetric Undo/Redo actions with the focused SwiftUI document `UndoManager`. Save captures the current bytes, edit list, revision, password, and export policy, then `PDFManualSavePreparationService` applies the edits in deterministic page/path order to an independent session in a detached task. Save As on macOS presents `NSSavePanel` and starts the same isolated candidate preparation while the panel remains open. Cancelling the panel cancels that pending result and leaves the edits and displayed document untouched.
8. When the macOS panel confirms, `beginDocumentDestinationAdoption` captures the current `NSDocument.fileURL`, file-modification date, `saveURL`, window title, and represented URL. It then synchronously assigns the selected destination to the native document and `saveURL`, synchronizes the window controller, and updates the title, represented URL, and AppKit Window menu item. This happens before awaiting candidate preparation or calling the file writer. Preparation or write failure invokes `rollbackDocumentDestinationAdoption`, restoring every captured identity value. On iOS, Save As uses the SwiftUI PDF file exporter with presentation state separate from the pending candidate.
9. Save replaces the active file through `ManualPDFSaveCoordinator`, which uses security-scoped access, `NSFileCoordinator`, and an atomic `Data.write`. A verified candidate is written before its pages/session are installed into the displayed document. After a successful macOS Save As write, the adopted native document receives the destination's modification date. Once presentation installation succeeds, `markManuallySaved(data:updatesFileDocumentSnapshot:)` updates working `sourceData`, clears the unsaved flag, and advances `persistedData` because subsequent document writes now target the adopted copy. If native destination adoption is unavailable, an existing document advances `persistedData` only when the destination resolves to its original `DocumentGroup` URL, preventing an exported copy from being returned later for an automatic write to the original file. An untitled document has no original file at risk and may advance its snapshot.
...
5. Before accepting a page-content rewrite, serialize and reopen the PDF, then compare every non-target text object's recursive path and exact UTF-16 content along with the existing sampled color metrics. Independently inspect the page resource dictionary: any named ColorSpace, Pattern, or Shading entry sends the edit directly to the appearance-safe PDFKit path, and an accepted candidate must have resource counts greater than or equal to the source counts. If regeneration changes unrelated text (including subset-font character mappings), materially changes color, or could discard special resources, roll back and add an independent white Square mask followed by a transparent PDFKit FreeText replacement. The FreeText baseline comes from the original object's composed transform and remains fixed; only the mask is trimmed above the nearest lower text object. The replacement includes measured line height, PDFKit inset, and width tolerance so a nominally single-line replacement does not wrap or clip.
...
- Annotation and page-object selection are simultaneously available without toolbar mode switches. Comment or E-sign placement takes precedence, followed by annotation hit testing and then page-object hit testing; selecting one target clears the other.
...
- `AnnotationRoundTrip.swift`: creates selectable text on two lines, calls the production highlight service, changes the Highlight to green, verifies its alpha and eight annotation-local quadrilateral points, and verifies highlight serialization/reopen properties alongside note, free-text, an appearance-safe CJK replacement, annotation-local Ink geometry, two independently placed multi-stroke signature Ink annotations, a checkmark, and a crossmark. It asserts that the replacement bounds include the styled font's actual line height plus PDFKit inset and verifies its contents after serialization/reopen. It also confirms that E-sign bounds are smaller than their placement containers, every path remains annotation-local, and a signature keeps its blue color and six-point line width after a second serialization/reopen cycle.
...
- `OBMColorSafetyValidation.swift`: applies a title replacement through the production background Save-preparation path, requires the special-resource fallback, verifies ColorSpace/Pattern/Shading resources do not decrease, compares before/after chromatic pixel coverage, and reopens the output to verify the replacement annotation.
...
- The standalone annotation round trip serialized and reopened a CJK appearance-safe replacement with a separate mask and transform-derived baseline; its Poppler render showed the full glyphs without clipping. Exact weight/slant remain manual visual checks.
- On 2026-08-28, the production background Save-preparation path was exercised against `OBM Timetable 2026.08.29.pdf`. Its first page retained two ColorSpace and eight Pattern resources and exactly 10,552 chromatic pixels. The input SHA-256 was unchanged, the generated copy had a different hash, and the replacement baseline measured 563.04 points versus the source 563.03998 points; preparation took about 10.95 seconds after the save destination had already been selected. Standalone destination-policy validation also preserved original bytes while writing distinct copy bytes. The final fixture-free PDFiumBridge run executed 32 tests with 31 passes, one intentional local-fixture skip, and no failures; unsigned Debug builds succeeded for macOS and the generic iOS Simulator.
...
The app first attempts an in-place rewrite only when font coverage, shaping policy, requested bold/italic state, and page-resource preflight permit it. The CoreText overlay path favors visible correctness and searchability when the original font cannot safely represent the requested text or the style changes. Bold and italic are synthetic for overlay text because only a regular CJK fallback face is bundled. Pages with named ColorSpace, Pattern, or Shading resources fail closed to a white Square mask plus transparent FreeText annotation, and every accepted candidate must retain at least the source resource counts. The FreeText lower edge is calculated from the source transform baseline and PDFKit's appearance inset; an independent mask handles original-content coverage and lower-neighbor avoidance. These fallbacks change the internal object structure and intentionally retain the original underlying text, but avoid regenerating unrelated page content or special color resources.
