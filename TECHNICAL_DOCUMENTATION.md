...
On 2026-09-05, the iOS document-owned Undo manager and `DocumentTitleMenuDisabler` passed iOS/macOS Swift syntax parsing, a focused iOS helper type check, and `git diff --check`. Earlier device checks showed that clearing only the rename delegate disabled the control without removing its appearance; the current implementation also clears the title-menu provider and document properties. No Xcode Build/Test or simulator validation was run. The final absence of the title-menu/Rename button and the responsiveness of the transition back to the document browser remain physical-device checks.
