...
| `PDF Editor/FeatureViews.swift` | Pages panel and narrow viewing rail with display, zoom, direct page-number, total-page, and separate previous/next controls; English tool panels whose E-sign category exposes Fill in form fields, Add a signature, Add a checkmark, and Add a crossmark without Request e-signatures; Protect PDF and Remove Password actions plus the protection password/confirmation form; reusable signature library, preview, deletion, and drawing pad; compact cross-platform PDF-image export options; multi-line Add Comment and Comment Editor views; Comment List; protected-merge password sheet; OCR result views; object inspector; and annotation inspector. |
...
7. Leaving an inline text editor records or removes a `PendingTextEdit` in `PendingTextEditStore`; it does not invoke PDFium. The store registers symmetric Undo/Redo actions with the focused SwiftUI document `UndoManager`. Save captures the current bytes, edit list, revision, authorized password, removal policy, and pending protection password, then `PDFManualSavePreparationService` applies text edits in deterministic page/path order to an independent session in a detached task. If protection was requested, PDFKit writes a temporary password-protected candidate and the service verifies its encryption, lock, rejection of an unrelated password, and acceptance of the requested password. Save As on macOS presents `NSSavePanel` and starts the same isolated candidate preparation while the panel remains open. Cancelling the panel cancels that pending result and leaves the edits and displayed document untouched.
...
9. Save of an existing file on macOS stages the verified candidate as the `ReferenceFileDocument` snapshot and asks the window's native `NSDocument` to perform the write. If the native write fails, the previous snapshot is restored. This keeps the app's own Save inside the document's coordinated write lifecycle instead of reporting an atomic replacement back to the same document as an external file change. Save As continues to use `ManualPDFSaveCoordinator`, which uses security-scoped access, `NSFileCoordinator`, and an atomic `Data.write`. A verified candidate is written before it is installed. A protection-only or password-removal candidate replaces the active editing session, working bytes, authorized password, and published security state but retains the content-identical `PDFDocument` pages already displayed by PDFKit; this avoids a presentation reload for a change that has no visual page-content effect. The document separately retains the presentation password. If a later annotation operation serializes the retained PDFKit document, that data is converted to the active session's encrypted or unencrypted policy before PDFium verification, so the prior security dictionary cannot return. Encrypted serialized and verification documents are explicitly unlocked before annotation inspection. Content-changing candidates retain the normal presentation synchronization path. After a successful macOS Save As write, the adopted native document receives the destination's modification date. Once presentation installation succeeds, `markManuallySaved(data:updatesFileDocumentSnapshot:)` updates working `sourceData`, clears the unsaved flag, and advances `persistedData` because subsequent document writes now target the adopted copy. If native destination adoption is unavailable, an existing document advances `persistedData` only when the destination resolves to its original `DocumentGroup` URL, preventing an exported copy from being returned later for an automatic write to the original file. An untitled document has no original file at risk and may advance its snapshot.
...
- On 2026-08-29, the macOS startup-window fill was deferred by one main-actor turn and passed `git diff --check`. Confirming that File → Open retains the normal arrow cursor during and after the native selection panel remains a manual macOS UI check; no app build or UI test was run for this change.
