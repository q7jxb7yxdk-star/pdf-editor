...
- Digital Signature Field uses the same direct placement, geometry, deletion, Undo/Redo, and canonical registration transaction with a 180 × 50-point default. It writes `/FT /Sig` and deliberately omits `/V` and `/DV`, so the Widget remains unsigned until an external signer fills it. Signature fields use generated `SignatureN` names, are excluded from value-edit controls, and do not trigger invalidation consent until a completed signature dictionary and ByteRange are present.
