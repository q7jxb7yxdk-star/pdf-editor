...
`updateAuthoredTextboxSize` measures explicit lines at the stored PDF font size. Width is the greater of 100 PDF points or the rounded widest line plus six points, capped by the distance from the field's left edge to the crop-box right edge. It then measures wrapping at that width. Empty explicit lines count as one line. Height uses the rounded font line height times the measured line count, without added vertical padding, subject to the existing minimum dimension and crop-box clamp. The left and top edges are retained where page bounds permit; deleting content can shrink both dimensions. Editor frames retain horizontal insets and zero vertical inset. Committing passes the text and fitted bounds through `onCommitTextFormField` to the document transaction.
