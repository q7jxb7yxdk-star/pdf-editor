...
- On 2026-09-06, an iPhone Instruments trace showed that the prior SwiftUI Save As exporter path completed its outer preparation in 27.81 ms, including 24.92 ms for `PrepareManualSave` and 9.11 ms in PDFium-locked preparation; it was therefore not a PDFium stall despite the endlessly visible toolbar spinner. iOS Save As now stages the verified candidate in a unique temporary directory and presents `UIDocumentPickerViewController(forExporting:asCopy:)` through `NativePDFExportPresenter`, removing that directory on success or cancellation. `git diff --check` and Swift syntax parsing passed. The native picker, selected-destination persistence, cancellation, and follow-up Save require manual iPhone verification; no Xcode Build/Test was run.
