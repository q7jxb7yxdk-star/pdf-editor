...
- Toggle the Pages thumbnail panel immediately to the left of the narrow right-side viewing rail. The panel opens and closes without a transition animation so PDFKit receives one layout-size change instead of repeatedly auto-scaling and redrawing intermediate frames. Select a thumbnail to navigate, drag thumbnails to reorder pages, extract that page as a standalone PDF, rotate it left or right, or delete it except when it is the final remaining page. Each thumbnail places Extract immediately before Rotate left. iOS exposes the standard Edit control for list reordering; macOS uses the list's direct move interaction. After the rail's Zoom out control, a 44-point page-number field, a 44-point total-page display, and separate Previous page and Next page buttons provide direct page navigation; page changes originating in PDFKit remain synchronized with the field.
...
- Fill existing AcroForm text fields, checkboxes, radio-button groups, combo boxes, and list boxes directly through PDFKit's native Widget controls. Widget fields are excluded from the app's general annotation move/delete gestures. A committed change is compared with the PDFium working copy, or the last accepted source bytes when no session exists, registered with Undo, marked unsaved, and serialized and verified through the current encryption policy. PDFium is retained only when its export also passes field and page checks; a PDFKit-compatible form can otherwise remain editable without a PDFium session. Synchronization retains the displayed document and pages instead of reloading the view after each field edit. Save and Save As repeat synchronization for active editors. Read-only fields remain read-only. XFA forms, PDF JavaScript/calculation/submit actions, AcroForm authoring, and cryptographic signing of Signature Widgets are not supported.
...
- Bookmark editing requires an unencrypted PDF and a classic cross-reference structure supported by the incremental writer. Only outline titles, hierarchy, and page destinations are represented by that writer; arbitrary outline actions, styling, and exact destination modes are not losslessly preserved. Candidates must pass the existing page, annotation, field, and bookmark checks, but those checks do not establish pixel-identical rendering for every PDF.
- Debug builds capture three local PDF snapshots if PDFium rejects both an incremental bookmark update and its PDFKit fallback rewrite. The error gives a unique temporary-directory path. These snapshots contain full document contents and filled form values; they are not uploaded, are not automatically deleted by the app, and may be removed by the system. Release builds exclude this capture code. Treat retained or shared diagnostic copies as sensitive.
