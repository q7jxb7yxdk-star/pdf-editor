...
- Start each document with the left Tools panel hidden and toggle it from the toolbar. In wide layouts, this toggle changes the available PDF width without a transition animation so PDFKit does not repeatedly auto-scale and redraw the page while the panel opens or closes. Save, Save As, Undo, Redo, Tools, and OCR are separate leading toolbar items with hidden shared backgrounds. The initial document title comes from `DocumentGroup`; macOS PDF document windows apply a one-time, non-animated fill frame on the next main-actor turn after the window attaches, avoiding a centered-to-filled startup jump and later file-switch movement without resizing during the native Open panel transition. macOS Save As immediately rebinds the native document to the selected destination when the filename panel confirms, so the window title, Window menu, represented URL, and subsequent Save target all follow the new filename without waiting for PDF regeneration. If saving fails, those values return to the previous document. The English tool workspace groups text/annotation, organization, export, e-signature, and security actions. The E-sign category exposes Fill in form fields, Add a signature, Add a checkmark, and Add a crossmark; Request e-signatures is not listed. Secure PDF exposes Protect PDF. All five categories start expanded when no preference has been stored; a disclosure click persists that category selection for later Tools panels and app launches, while ignoring stored category names the current workspace does not recognize.
...
- The deferred macOS document-window fill passed `git diff --check` on 2026-08-29. Confirming that File → Open retains the normal arrow cursor during and after the native selection panel remains a manual macOS UI check.
