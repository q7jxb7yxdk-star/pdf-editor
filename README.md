...
- Open, display, edit, and create PDF documents through SwiftUI's document architecture. Edits remain in memory until the user selects the cross-platform Save toolbar action. Pending text edits are prepared and verified on an isolated background candidate, keeping expensive PDF regeneration off the main actor. On macOS, Save As presents its native filename panel immediately and prepares the candidate while the panel is open. When the user confirms a destination, the same window synchronously adopts that URL and updates its title, Window menu item, and represented file before awaiting candidate preparation or writing the verified copy. A failed preparation or write restores the previous document identity; after success, later edits and Save operations target the new copy while the original remains unchanged. Save As is also available from the toolbar on iOS/iPadOS, and macOS additionally provides File → Save, File → Save As…, Command-S, and Shift-Command-S.
- Start each document with the left Tools panel hidden and toggle it from the toolbar. In wide layouts, this toggle changes the available PDF width without a transition animation so PDFKit does not repeatedly auto-scale and redraw the page while the panel opens or closes. Save, Save As, Undo, Redo, Tools, and OCR are separate leading toolbar items with hidden shared backgrounds. The initial document title comes from `DocumentGroup`; macOS PDF document windows apply a one-time, non-animated fill frame as soon as the window attaches, avoiding a centered-to-filled startup jump and later file-switch movement. macOS Save As immediately rebinds the native document to the selected destination when the filename panel confirms, so the window title, Window menu, represented URL, and subsequent Save target all follow the new filename without waiting for PDF regeneration. If saving fails, those values return to the previous document. The English tool workspace groups text/annotation, organization, export, e-signature, and security actions. The E-sign category exposes Add a signature, Add a checkmark, and Add a crossmark; Fill & Sign and Request e-signatures are not listed. All five categories start expanded when no preference has been stored; a disclosure click persists that category selection for later Tools panels and app launches, while ignoring stored category names the current workspace does not recognize.
...
- Unlock encrypted PDFs and preserve encryption during ordinary saves. Empty password submissions are ignored, and locked documents create the PDFium editing session only after a non-empty password is accepted. Password removal is an explicit save option for an already unlocked document.
...
- Existing text replacement can change implementation strategy: unsupported glyph coverage or complex shaping uses a new searchable CoreText overlay rather than rewriting the original text object with its font. The normal overlay path makes the original object invisible and blanks its text before importing the replacement. Before accepting either rewrite, the PDFium bridge serializes and reopens the page, then verifies every non-target text object's path and exact UTF-16 content as well as sampled color metrics. A page whose resources contain named ColorSpace, Pattern, or Shading entries is sent directly to the appearance-safe path, and accepted output must not reduce any of those resource counts. If regeneration changes unrelated subset-font text or could discard special color resources, the app rolls back and creates a white mask plus an editable, transparent FreeText visual replacement. The FreeText lower edge is derived from the original object's transform baseline; the independent mask can be trimmed above nearby lower text without pushing the replacement upward. PDFKit layout tolerance prevents a single-line replacement from wrapping or clipping. That integrity fallback leaves the original content-stream text underneath the annotations, so searches may still find the original text.
- Inline text changes are pending view state until Save. Closing the document without saving discards them. Save preparation runs on an isolated background document and atomically installs the verified candidate, but file coordination and final UI interaction still occur in their platform save paths. A successful appearance-safe fallback no longer presents an informational alert. On macOS, the short-lived fallback used before a page overlay mounts is noninteractive and is removed or migrated when its page overlay becomes available, or when scrolling or document replacement occurs. Its mask is aligned to backing pixels without a fixed horizontal expansion to avoid a vertical seam. The app has no automated UI test proving exact font metrics, save-panel interaction, or scrolling behavior for every PDF producer and embedded-font format.
...
- `PDFiumBridge` package tests: 32 tests executed with 31 passes, 1 skipped local-fixture test, and 0 failures in the final fixture-free run. An earlier run with the then-available real subset-font integrity fixture passed all 32 tests.
...
- The macOS native Save As panel path, independent iOS exporter presentation state, and CJK FreeText fallback sizing passed Debug builds for macOS and the generic iOS Simulator destination on 2026-08-28. The standalone annotation round trip serialized, reopened, and visibly rendered the replacement text without clipping. Actual Save As panel interaction remains a manual UI check.
- The immediate-panel Save As flow, isolated `ReferenceFileDocument` snapshot policy, background Save candidate, special-resource guard, and transform-baseline FreeText fallback passed unsigned Debug builds for macOS and the generic iOS Simulator destination on 2026-08-28. Standalone destination validation kept the original bytes unchanged while writing a distinct copy. A production-path run against `OBM Timetable 2026.08.29.pdf` preserved two ColorSpace and eight Pattern resources, retained exactly 10,552 chromatic pixels, kept the input SHA-256 unchanged, and aligned the replacement baseline at 563.04 points versus the source 563.03998 points. Runtime native-panel timing and automatic document lifecycle behavior remain manual UI checks.
